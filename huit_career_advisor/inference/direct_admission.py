...
def _load_tt_model():
    ...
    if _TT_MODEL is not None:
        return _TT_MODEL
    try:
        path = model_path('tt_models.pkl')
        if path.exists():
            _TT_MODEL = joblib.load(path)
        else:
            _TT_MODEL = None
    except Exception:
        _TT_MODEL = None
...
def _group_mapping_codes():
    ...
    return DIRECT_ADMISSION_GROUP_MAPPING
    ...
    try:
        ...
        if model:
            ...
            return [{
                ...
                'xac_suat': 0.0,
                'ly_do': 'Vui lòng chạy research/training/train_tuyenthang_models.py'
            }]
...
def demo():
    """Run the historical command-line smoke demo."""
